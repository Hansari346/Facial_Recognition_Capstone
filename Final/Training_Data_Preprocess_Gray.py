...
def concatenate_training_data(student_pic_path):
    ...
    for category in CATEGORIES:
        ...
        for img in os.listdir(path): #iterate through each file in the directory path
            ...
            new_array = cv2.resize(img_array, (Width, Height))	#resize to 100x100       
...
x_train = np.array(x_train).reshape(-1,Width, Height) #-1 because gray scale
y_train = np.array(y_train)
x_test = np.array(x_test).reshape(-1,Width, Height) #-1 because gray scale
